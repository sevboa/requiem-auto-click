...
class SharpeningManager:
    ...
    def __init__(
        ...
        *,
        ...
        # Прогреваем кэш всех шаблонов заточки заранее:
        # - окно, авто-активно, safe sharpening, reject popup
        # - '+' (5 вариантов)
        # - цифры 0..9 для каждого варианта (5*10)
        digits_dir = self._ASSETS_DIR / "digits"
        variants = [f"a{i}" for i in range(1, 6)]
        preload_templates(
            [
                self.TEMPLATE_WINDOW_OPENED,
                self.TEMPLATE_AUTO_ACTIVE,
                self.TEMPLATE_SAVE_SHARPENING,
                self.TEMPLATE_REJECT_OK,
                *[digits_dir / f"+_{v}.png" for v in variants],
                *[digits_dir / f"{d}_{v}.png" for v in variants for d in range(10)],
            ]
        )
        ...
        if cache_on_init:
            self.ensure_window_cached(threshold=0.98, timeout_s=0.0, poll_s=0.1)
    ...
    def drag_item_from_backpack_cell_to_sharpening_cell(
        ...
        *,
        ...
        self._clicker.drag_client(start_client, target_client, steps=50, step_delay=0.007)
        return True
    ...
    def _get_plus_variant_in_window(
        ...
        *,
        ...
        if plus_gray is None:
            return None
        # Защита от "обрезанного/пустого" ROI: иначе match может давать мусорные совпадения.
        try:
            h, w = plus_gray.shape[:2]
        except Exception:
            return None
        if int(h) < int(self.PLUS_ROI_SIZE[1]) or int(w) < int(self.PLUS_ROI_SIZE[0]):
            return None
    ...
    def get_current_sharpening_value(
        ...
        *,
        ...
        # Защита от "обрезанного/пустого" ROI: если массив меньше ожидаемого — это ошибка,
        # иначе возможно мусорное распознавание (например 99) на пустом/битом кадре.
        try:
            h, w = digits_gray.shape[:2]
        except Exception as e:
            raise RuntimeError(f"Не удалось прочитать ROI цифр уровня заточки: {e}") from e
        need_w, need_h = self.SHARPENING_DIGITS_ROI_SIZE
        if int(h) < int(need_h) or int(w) < int(need_w):
            raise RuntimeError(
                f"ROI цифр уровня заточки меньше ожидаемого: got=({w}x{h}) need=({need_w}x{need_h})"
            )
