...
class BackpackManager:
    ...
    def close_all_opened_backpacks(
        ...
        *,
        ...
        """
        Закрывает все открытые рюкзаки кликом в центр иконок.
        ...
        for s in states:
            if s.get("state") != "opened":
                continue
            x0, y0 = s["roi_top_left_client"]
            w, h = s["roi_size"]
            cx = int(x0 + w // 2)
            cy = int(y0 + h // 2)
            self._clicker.click_at_client(cx, cy)
            time.sleep(0.2)
    ...
    def open_backpack(
        ...
        *,
        ...
        """
        Открывает конкретный рюкзак по индексу (0..7) кликом в центр иконки.
