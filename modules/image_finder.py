...
from modules.window_utils import find_hwnd_by_title_substring, client_to_screen, SW_RESTORE
...
def _load_template_gray_and_mask_cached(template_png_path: str, alpha_threshold: int = 10):
    """
    Кэшированная загрузка шаблона (gray + mask).

    Важно: чтение PNG с диска — дорогая операция; для частых матчей (цифры, индикаторы)
    выгодно держать шаблоны в памяти.
    """
...
class ImageFinder:
    ...
    @staticmethod
    def _load_template_gray_and_mask(template_png_path: str, alpha_threshold: int = 10):
        tpl = _cv2.imread(template_png_path, _cv2.IMREAD_UNCHANGED)  # type: ignore[attr-defined]
        if tpl is None:
            raise FileNotFoundError(f"Template unreadable: {template_png_path}")
    ...
    def find_template_in_client_roi(
        ...
        *,
        ...
        tpl_gray, mask = _load_template_gray_and_mask_cached(template_path, int(alpha_threshold))
